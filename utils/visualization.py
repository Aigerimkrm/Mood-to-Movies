...
def get_palette_for_emotion(emotion):
    return colors.get(emotion, [(0, 0, 0)])

def display_gradient_palette(emotion):
    from PIL import Image
    palette = get_palette_for_emotion(emotion)
    width, height = 300, 50
...
def create_collage_from_urls(urls, target_size=(400, 225), rows=2, cols=5):
    images = []
